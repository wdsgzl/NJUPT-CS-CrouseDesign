...
class Sample:
    ...
    def main(
        ...
        pub_request = iot_20180120_models.PubRequest(
            ...
            product_key='k1ggwzwH4wE',
            device_name='CCOO',
            topic_full_name='/k1ggwzwH4wE/CCOO/user/get',
...
if __name__ == '__main__':
    ...
    cur = conn.cursor()
    cur.execute("select * from Dio order by time desc limit 1")
    ...
    for row in cur.fetchall():  # 以一条记录为元组单位返回结果给row
        current_c = row[1]
    if past_c == current_c:
        cur.execute("insert into Dio Values('二氧化碳浓度','{data}','{CO}','{status}')".format(
            data=time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()), CO=float(0), status='False'))
