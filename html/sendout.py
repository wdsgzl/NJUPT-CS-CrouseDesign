...
class Sample:
    ...
    def main(
        ...
        pub_request = iot_20180120_models.PubRequest(
            ...
            product_key='k1ggwxWz3Re',
            device_name='wendu',
            topic_full_name='/k1ggwxWz3Re/wendu/user/get',
...
if __name__ == '__main__':
    ...
    cur.execute("select * from Tem order by time desc limit 1")
    ...
    con = sqlite3.connect('../mytest/db.sqlite3')  # 连接数据库
    curr = con.cursor()
    curr.execute("select * from Tem order by time desc limit 1")
    for row in curr.fetchall():  # 以一条记录为元组单位返回结果给row
        current_t = row[1]
    print(past_t)
    print(current_t)
    if past_t == current_t:
        curr.execute("insert into Tem Values('温感','{data}','{tem}','{status}')".format(
            data=time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()), tem=float(0), status='False'))
    con.commit()
    con.close()
